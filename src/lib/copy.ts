...
function buildCopy(locale: 'fr' | 'en' | 'es'): CopyKeys {
  ...
  return {
    ...
    pricing: {
      ...
      features: { pages: isFr ? 'pages' : 'pages', articles: isFr ? 'articles SEO/mois' : isEn ? 'SEO & GEO articles/month' : 'artículos SEO/mes', booking: isFr ? 'Réservation en ligne' : isEn ? 'Online booking' : 'Reservas en línea', payments: isFr ? 'Paiements en ligne' : isEn ? 'Online payments' : 'Pagos en línea', products: 'products', gmb: 'Google Business', support: isFr ? 'Support prioritaire' : isEn ? 'Priority support' : 'Soporte prioritario', ssl: 'SSL', hosting: isFr ? 'Hébergement inclus' : isEn ? 'Hosting included' : 'Hosting incluido' },
    ...
    footer: { description: isFr ? 'Mindzy – Agence digitale spécialisée en développement web, SEO et visibilité dans les intelligences artificielles (GEO).' : isEn ? 'Mindzy – Digital agency specialized in web development, SEO and visibility in artificial intelligences (GEO).' : 'Mindzy – Agencia digital especializada en desarrollo web, SEO y visibilidad en las inteligencias artificiales (GEO).', sections: { solutions: 'Solutions', resources: isFr ? 'Ressources' : isEn ? 'Resources' : 'Recursos', company: isFr ? 'Entreprise' : isEn ? 'Company' : 'Empresa', legal: 'Legal' }, links: { siteWeb: isFr ? 'Site web' : isEn ? 'Website' : 'Sitio web', surMesure: isFr ? 'Solution sur mesure' : isEn ? 'Custom solution' : 'Solución a medida', formations: isFr ? 'Formations' : isEn ? 'Training' : 'Formación', branding: isFr ? 'Branding & Contenu' : isEn ? 'Branding & Content' : 'Branding y Contenido', portfolio: 'Portfolio', blog: 'Blog', faq: 'FAQ', about: isFr ? 'À propos' : isEn ? 'About' : 'Sobre nosotros', whyUs: isFr ? 'Pourquoi nous' : isEn ? 'Why us' : 'Por qué nosotros', reviews: isFr ? 'Avis' : isEn ? 'Reviews' : 'Opiniones', cgu: 'CGU', cgv: 'CGV', mentions: isFr ? 'Mentions' : 'Legal' }, copyright: isFr ? 'Tous droits réservés.' : isEn ? 'All rights reserved.' : 'Todos los derechos reservados.' },

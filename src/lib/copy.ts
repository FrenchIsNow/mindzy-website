...
interface CopyKeys {
  ...
  pricing: { title: string; subtitle: string; monthly: string; setup: string; cta: string; popular: string; customQuote: { title: string; description: string; cta: string }; roi: { title: string; sessionPrice: string; result: string; resultText: string }; features: Record<string, string>; plans: Record<string, { name: string; description: string }> }
  ...
  resources: { title: string; subtitle: string; download: string; emailPlaceholder: string; emailCta: string }
  blog: { title: string; subtitle: string; readMore: string; readTime: string; categories: Record<string, string> }
  faq: { title: string; subtitle: string; categories: Record<string, string> }
...
function buildCopy(locale: 'fr' | 'en' | 'es'): CopyKeys {
  ...
  return {
    ...
    pricing: {
      ...
      setup: 'Setup',
      cta: isFr ? 'Choisir cette offre' : isEn ? 'Choose this plan' : 'Elegir este plan',
      ...
      roi: { title: isFr ? 'Calculateur de ROI' : isEn ? 'ROI Calculator' : 'Calculadora de ROI', sessionPrice: isFr ? 'Prix moyen d\'une prestation (€)' : isEn ? 'Average service price (€)' : 'Precio medio por servicio (€)', result: isFr ? 'clients/mois' : isEn ? 'clients/month' : 'clientes/mes', resultText: isFr ? 'suffisent pour rentabiliser Mindzy' : isEn ? 'enough to recoup Mindzy cost' : 'suficientes para rentabilizar Mindzy' },
      ...
      plans: { basic: { name: isFr ? 'Starter' : isEn ? 'Starter' : 'Starter', description: isFr ? 'Pour démarrer votre présence en ligne' : isEn ? 'To start your online presence' : 'Para iniciar tu presencia en línea' }, pro: { name: 'Pro', description: isFr ? 'Pour développer votre visibilité' : isEn ? 'To develop your visibility' : 'Para desarrollar tu visibilidad' }, business: { name: 'Business', description: isFr ? 'Pour automatiser votre activité' : isEn ? 'To automate your business' : 'Para automatizar tu negocio' }, ecommerce: { name: 'E-commerce', description: isFr ? 'Pour vendre en ligne' : isEn ? 'To sell online' : 'Para vender en línea' } },
    },
    diagnostic: {
    ...
    resources: { title: isFr ? 'Ressources gratuites' : isEn ? 'Free resources' : 'Recursos gratuitos', subtitle: isFr ? 'Guides et outils pour votre présence en ligne' : isEn ? 'Guides and tools for your online presence' : 'Guías y herramientas para tu presencia en línea', download: isFr ? 'Télécharger' : isEn ? 'Download' : 'Descargar', emailPlaceholder: isFr ? 'Votre email' : isEn ? 'Your email' : 'Tu email', emailCta: isFr ? 'Recevoir les ressources' : isEn ? 'Get resources' : 'Recibir recursos' },
    blog: { title: 'Blog', subtitle: isFr ? 'Conseils pour développer votre activité' : isEn ? 'Tips to grow your business' : 'Consejos para desarrollar tu negocio', readMore: isFr ? 'Lire la suite' : isEn ? 'Read more' : 'Leer más', readTime: isFr ? 'min de lecture' : isEn ? 'min read' : 'min de lectura', categories: { all: isFr ? 'Tous' : isEn ? 'All' : 'Todos', marketing: 'Marketing', seo: 'SEO', productivite: isFr ? 'Productivité' : isEn ? 'Productivity' : 'Productividad' } },
    faq: { title: isFr ? 'Questions fréquentes' : isEn ? 'FAQ' : 'Preguntas frecuentes', subtitle: isFr ? 'Tout ce que vous devez savoir avant de démarrer' : isEn ? 'Everything you need to know before getting started' : 'Todo lo que necesitas saber antes de empezar', categories: { all: isFr ? 'Toutes' : isEn ? 'All' : 'Todas', general: isFr ? 'Général' : 'General', pricing: isFr ? 'Tarifs' : 'Pricing', technical: isFr ? 'Technique' : 'Technical', support: 'Support', features: isFr ? 'Fonctionnalités' : isEn ? 'Features' : 'Funcionalidades', process: isFr ? 'Processus' : isEn ? 'Process' : 'Proceso' } },
    ...
    footer: { description: isFr ? 'Mindzy crée des sites web premium pour les entrepreneurs.' : isEn ? 'Mindzy creates premium websites for entrepreneurs.' : 'Mindzy crea sitios premium para emprendedores.', sections: { product: isFr ? 'Produit' : 'Product', resources: isFr ? 'Ressources' : 'Resources', company: isFr ? 'Entreprise' : 'Company', legal: 'Legal' }, links: { pricing: isFr ? 'Tarifs' : 'Pricing', portfolio: 'Portfolio', process: isFr ? 'Processus' : 'Process', diagnostic: 'Diagnostic', blog: 'Blog', academy: 'Academy', resources: isFr ? 'Ressources' : 'Resources', faq: 'FAQ', about: isFr ? 'À propos' : 'About', whyUs: isFr ? 'Pourquoi nous' : 'Why us', reviews: isFr ? 'Avis' : 'Reviews', cgu: 'CGU', cgv: 'CGV', mentions: isFr ? 'Mentions' : 'Legal' }, copyright: isFr ? 'Tous droits réservés.' : isEn ? 'All rights reserved.' : 'Todos los derechos reservados.' },

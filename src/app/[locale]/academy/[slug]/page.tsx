  ...
  return (
    <div className="pt-32 pb-20">
      <article className="container-narrow">
        <div className="mb-8"><Link href={`/${locale}/academy`}><Button variant="ghost" size="sm">← {getMessages(locale as Locale).common.back}</Button></Link></div>
        ...
        <footer className="mt-12 pt-8 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4"><p className="text-gray-600">Prêt à passer à l&apos;action ?</p><Link href={`/${locale}/diagnostic`}><Button variant="primary" size="md">{getMessages(locale as Locale).hero.cta}</Button></Link></footer>
      </article>
    </div>
  )
}

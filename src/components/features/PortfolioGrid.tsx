...
export function PortfolioGrid({ locale }: { locale: Locale }) {
  ...
  const filters: { value: CategoryFilter; label: string }[] = [
    ...
    { value: 'therapist', label: t.filter.therapist },
    { value: 'clinic', label: t.filter.clinic },
    { value: 'coach', label: t.filter.coach },
    { value: 'ecom', label: t.filter.ecom },
  ]
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.map((item) => (
          <Card key={item.id} variant="outline" hover className="overflow-hidden">

...
interface FormationsFeatureGridProps {
  features: Feature[]
}

export function FormationsFeatureGrid({ features }: FormationsFeatureGridProps) {
  return (
          ...
          {features.map((feature, i) => (
            ...
            >
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-violet-100 to-violet-50 flex items-center justify-center text-violet-600 mb-5">
                <FormationIcon name={feature.icon} />
              </div>
...
function FormationIcon({ name }: { name: string }) {
  const cls = 'w-5 h-5'
  switch (name) {
    case 'linkedin':

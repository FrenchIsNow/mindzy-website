...
const TRANSLATIONS = {
  en: {
    ...
    h2line2: 'AI infrastructure that runs your entire company.',
    p1: 'An AI agent handles one task. An AI infrastructure runs your entire company. Mindzy designs the operating layer that connects every model, every tool, every department, and every workflow into a coherent system you actually control.',
  ...
  fr: {
    ...
    h2line2: 'Une infrastructure IA qui fait tourner toute votre entreprise.',
    p1: 'Un agent IA gère une tâche. Une infrastructure IA fait tourner toute votre entreprise. Mindzy conçoit la couche opérationnelle qui connecte chaque modèle, chaque outil, chaque département et chaque flux de travail dans un système cohérent que vous contrôlez réellement.',
  ...
  es: {
    ...
    h2line2: 'Infraestructura IA que gestiona toda tu empresa.',
    p1: 'Un agente de IA gestiona una tarea. Una infraestructura de IA gestiona toda tu empresa. Mindzy diseña la capa operativa que conecta cada modelo, cada herramienta, cada departamento y cada flujo de trabajo en un sistema coherente que realmente controlas.',
  ...
  de: {
    ...
    h2line2: 'KI-Infrastruktur, die Ihr gesamtes Unternehmen führt.',
    p1: 'Ein KI-Agent erledigt eine Aufgabe. Eine KI-Infrastruktur führt Ihr gesamtes Unternehmen. Mindzy entwirft die Betriebsschicht, die jedes Modell, jedes Tool, jede Abteilung und jeden Workflow in ein kohärentes System verbindet, das Sie wirklich kontrollieren.',
  ...
  it: {
    ...
    h2line2: 'Infrastruttura IA che gestisce l\'intera tua azienda.',
    p1: 'Un agente IA gestisce un compito. Un\'infrastruttura IA gestisce l\'intera tua azienda. Mindzy progetta il livello operativo che connette ogni modello, ogni strumento, ogni reparto e ogni flusso di lavoro in un sistema coerente che controlli davvero.',
  ...
  pt: {
    ...
    h2line2: 'Infraestrutura IA que gerencia toda a sua empresa.',
    p1: 'Um agente de IA gerencia uma tarefa. Uma infraestrutura de IA gerencia toda a sua empresa. A Mindzy projeta a camada operacional que conecta cada modelo, cada ferramenta, cada departamento e cada fluxo de trabalho em um sistema coerente que você realmente controla.',
  ...
  ar: {
    eyebrow: 'ما يميزنا',
    h2line1: 'صُنع لك.',
    h2line2: 'بنية تحتية للذكاء الاصطناعي تدير شركتك بأكملها.',
    p1: 'وكيل الذكاء الاصطناعي يتعامل مع مهمة واحدة. البنية التحتية للذكاء الاصطناعي تدير شركتك بأكملها. تصمم Mindzy الطبقة التشغيلية التي تربط كل نموذج وكل أداة وكل قسم وكل سير عمل في نظام متماسك تتحكم فيه فعلاً.',
  ...
  zh: {
    eyebrow: '我们的与众不同之处',
    h2line1: '为你而生。',
    h2line2: '驱动整个企业运营的 AI 基础设施。',
    p1: 'AI 代理处理一项任务。AI 基础设施驱动您的整个企业。Mindzy 设计将每个模型、每个工具、每个部门和每个工作流程连接到您真正控制的连贯系统中的操作层。',
  ...
  ja: {
    eyebrow: '私たちの違い',
    h2line1: 'あなたのために。',
    h2line2: '企業全体を動かすAIインフラ。',
    p1: 'AIエージェントは一つのタスクを処理します。AIインフラはあなたの企業全体を動かします。Mindzyは、すべてのモデル、すべてのツール、すべての部門、すべてのワークフローを、あなたが実際にコントロールできる一貫したシステムに接続する運用レイヤーを設計します。',
  ...
  ru: {
    ...
    h2line2: 'ИИ-инфраструктура, которая управляет всей вашей компанией.',
    p1: 'ИИ-агент выполняет одну задачу. ИИ-инфраструктура управляет всей вашей компанией. Mindzy проектирует операционный слой, который соединяет каждую модель, каждый инструмент, каждый отдел и каждый рабочий процесс в единую систему, которую вы реально контролируете.',
